let DCONSTANTS = {
   ...
   PODS_TEXT1:'All pods have web & mobile UI built with React and Native React and most pods have admin interfaces.',
    ...
    SERVERLESS_TEXT1:'We’ve invested, and are going to invest some more in building a cutting edge architecture which simplifies implementation of complex functionalities for developers.',
    ...
    SPEED_TEXT_WHITE:'Ivory aims at “google fast” speeds of production by default. Due to the use of DynamoDB with DAX enabled by default and Appsync (GraphQL) most requests are handled with single digit milliseconds response time.',
    SPEED_TEXT_BLUE:'The web application, a single page React app, is served via CloudFront to reduce network latency and makes extensive use of server-side rendering, embedding fonts, scripts and resources so that when loading it the “time to first screen” is instantaneous.',
    SECURITY_TEXT_BLUE:'Absolutely all data stored and manipulated by Ivory is fully encrypted in transit and at rest. AWS KMS is used for encryption, decryption and key management.',
    SECURITY_TEXT_WHITE1:'Through the use of the User Authentication and Authorisation Pod the Ivory developers get access to a sophisticated user registration, authentication, roles & permission system. Standard functionality such as assigning individual user permissions, groups and audit log are part of the pod and can be accessed from the Admin section.',
    SECURITY_TEXT_WHITE2:'Security and privacy is audited by an independent security audit firm and the standard Ivory project template is pen-tested every quarter.',
    AI_TEXT_WHITE1:'In order to ensure that every application built with Ivory is future roof and ML/AI ready the platform employs a sophisticated microservices based architecture with an event bus (Kinesis) and event log (Dynamo). Essentially this means that every message going through the system (eg. "Product viewed", "Product added to cart", "Order placed", "Bill issued") is saved.',
    AI_TEXT_WHITE2:'Since data collection is a foundational step in a big data and AI strategy the core Ivory architecture allows for data to be collected to be used at some later time to train ML models.',
    BLOCKCHAIN_TEXT_BLUE1:'Ivory aims to accelerate the development of a typical Blockchain MVP by a factor of 10x. Ivory has native support for Blockchain and significantly accelerates Blockchain development in 3 areas:',
    BLOCKCHAIN_TEXT_BLUE2:'1) Devops - setting up the necessary infrastructure to run the EVM (Ethereum Virtual Machine) and create a private Ethereum chain is as simple as adding the Blockchain Pod;',
    BLOCKCHAIN_TEXT_WHITE3:'2) Solidity contract development - a started kit and template of not only creating and deploying the first Solidity contracts but also on how to integrate these into the application Pods;',
